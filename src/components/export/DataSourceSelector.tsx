...
import { Database, Search, Loader2, AlertCircle, Filter, Info, ChevronDown, ChevronUp } from 'lucide-react'
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-amber-800 dark:text-amber-200">
                {incompatibleSources.length} source{incompatibleSources.length !== 1 ? 's' : ''} filtered due to incompatible structure
              </span>
            </div>
            {showFilteredDetails ? (
                  ...
                  {incompatibleSources.slice(0, 10).map((source) => (
                    <li key={source.EntityTypeName} className="flex items-center gap-2">
                      ...
                      <span className="text-amber-600 dark:text-amber-500">
                        ({source.ObjectTypeName}, Parent: {source.PrimaryParentEntityTypeName || 'none'})
                      </span>
                    </li>
                  ))}
