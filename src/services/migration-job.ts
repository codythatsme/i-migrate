...
export class MigrationJobService extends Effect.Service<MigrationJobService>()("app/MigrationJobService", {
  accessors: true,
      ...
      Effect.try({
        try: () =>
          db
            .select()
            .from(failedRows)
            .where(eq(failedRows.jobId, jobId))
            .all(),
      ...
      Effect.gen(function* () {
        ...
        if (!sourcePassword) {
          return yield* Effect.fail(
            new MissingCredentialsError({ environmentId: sourceEnvId })
          )
        }
  ...
  static Test = Layer.succeed(
    ...
    new MigrationJobService({
      ...
      retryFailedRows: () =>
        Effect.succeed({ retriedCount: 0, successCount: 0, failCount: 0 }),
